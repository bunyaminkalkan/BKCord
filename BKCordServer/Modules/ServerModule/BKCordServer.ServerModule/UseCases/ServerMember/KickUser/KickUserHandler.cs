...
public class KickUserHandler : IRequestHandler<KickUserCommand>
{
    ...
    public KickUserHandler(AppServerDbContext dbContext, IHttpContextService httpContextService, IServerAuthorizationService serverAuthorizationService)
    {
    ...
    public async Task Handle(KickUserCommand request, CancellationToken cancellationToken)
    {
        ...
        await _serverAuthorizationService.ValidateUserHavePermissionByUserIdAndServerId(userId, request.ServerId, Contracts.RolePermission.KickMembers);
        ...
        serverMembersHistory.IsKicked = true;
